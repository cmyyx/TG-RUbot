...
- 🛡️ **人机验证系统** - 使用简单数学题防止垃圾信息和机器人滥用
- 🔄 **自动置顶续期** - 每周自动更新元数据消息，确保系统持续运行
- 💬 **命令提醒** - 新话题自动显示可用命令，提升使用体验
...
为了防止垃圾信息和自动化机器人滥用，系统现在支持人机验证功能：

* **自动挑战**: 当新访客首次发送消息时，机器人会自动发送一个简单的算术题（如 "3 + 5 = ?"）
* **三次机会**: 访客每天有三次机会回答验证问题
* **消息转发**: 即使访客未验证，所有消息仍会转发到超级群组，但不会通知管理员
* **表情标记**: 只有验证通过的访客消息才会添加表情反应标记（🕊）
* **每日重置**: 如果访客当天验证失败，第二天会获得新的挑战和三次新机会
* **自动封禁**: 连续两天验证失败的访客会被自动封禁
* **管理员控制**: 管理员可以随时使用 ban/unban 命令手动管理访客，无需等待验证完成

**使用示例**:
1. 新访客发送消息 → 收到验证挑战
2. 访客回答正确 → 验证通过，后续消息正常转发并通知管理员
3. 访客回答错误 → 收到新的挑战，最多三次机会
4. 三次都错误 → 当天无法验证，消息继续转发但不回复访客

### 🔄 自动置顶续期

为了确保系统持续可靠运行，实现了自动置顶消息续期功能：

* **定时检查**: 使用 Cloudflare Workers 的 Cron Triggers，每周日午夜自动执行
* **智能续期**: 当元数据置顶消息超过 6 天时，自动创建新消息并重新置顶
* **内容保持**: 续期过程完全保留原有元数据内容，不做任何修改
* **无需干预**: 完全自动化，无需手动操作

**配置说明**:

1. 系统已在 `wrangler.toml` 中配置了 Cron 触发器：
```toml
[triggers]
crons = ["0 0 * * 0"]  # 每周日午夜执行
```

2. **可选的环境变量**（在 Worker 设置中配置）：
   - `BOT_TOKEN`：您的机器人 API Token（从 @BotFather 获取）
   - `OWNER_UID`：您的 Telegram 用户 ID（从 @userinfobot 获取）

> **注意**: 如果不配置这两个环境变量，自动续期功能将无法工作，但不会影响其他功能的正常使用。

### 💬 命令提醒

为了提升用户体验，新话题创建时会自动显示可用命令：

* **自动发送**: 当为新访客创建话题时，机器人会自动发送命令说明
* **完整列表**: 包含所有可用命令（ban、unban、silent_ban、silent_unban）
* **易于复制**: 命令格式化为可点击复制的形式，方便快速使用
...
* 在**私聊对应的 Topic** 中（这些命令会在新话题创建时自动显示）：
...
* **人机验证**: 新访客首次发送消息时会收到验证挑战，需要正确回答简单的算术题才能正常通信
  * 未验证的访客消息会被转发到话题，但不会通知管理员，也不会添加表情标记
  * 管理员可以随时使用 ban/unban 命令管理访客，无需等待验证完成
  * 验证失败的访客消息仍会转发，但机器人不会回复访客
* **自动置顶续期**: 系统每周会自动检查并更新元数据置顶消息，确保长期稳定运行
...
**必需的环境变量**：
- `PREFIX`：URL 前缀，用于访问路径（例如：`public`）
- `SECRET_TOKEN`：安全令牌，必须至少16位，包含大小写字母和数字

**可选的环境变量**（用于启用自动置顶续期功能）：
- `BOT_TOKEN`：您的机器人 API Token（从步骤2中的 @BotFather 获取）
- `OWNER_UID`：您的 Telegram 用户 ID（从步骤1中的 @userinfobot 获取）

> [!TIP]
> 如果不配置 `BOT_TOKEN` 和 `OWNER_UID`，自动置顶续期功能将无法工作，但不会影响其他功能的正常使用。建议配置这两个变量以获得完整的功能体验。

您可以参考项目根目录的 `.env.example` 文件查看环境变量配置示例。
...
9. 配置**运行时**环境变量：
   ...
   - **（可选）** 如果需要启用自动置顶续期功能，还需添加：
     - 添加类型为**Secret**的 `BOT_TOKEN`（您的机器人 API Token）
     - 添加类型为**Plaintext**的 `OWNER_UID`（您的 Telegram 用户 ID）
...
3. 配置环境变量：
   ...
   - **（可选）** 如果需要启用自动置顶续期功能，还需添加：
     - `BOT_TOKEN`：您的机器人 API Token
     - `OWNER_UID`：您的 Telegram 用户 ID
